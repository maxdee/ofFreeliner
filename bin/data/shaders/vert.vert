...
void main()
{
    ...
    float displacementHeight = 40.0;
    float res = 50.0;

    float displacementY = sin(time + (position.x / res)) * displacementHeight;
    float displacementZ = sin(time + (position.y / 100.0)) * displacementHeight;


    vec4 modifiedPosition = position;
  	modifiedPosition.y += displacementY;
    modifiedPosition.x += displacementZ;
